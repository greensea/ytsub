...
BEGIN_EVENT_TABLE( GUIFrame, wxFrame )
    ...
    EVT_MENU(idMenuOpenFilm, GUIFrame::_wxFB_OnOpenFilm)
END_EVENT_TABLE()
...
GUIFrame::GUIFrame( wxWindow* parent, int id, wxString title, wxPoint pos, wxSize size, int style ) : wxFrame( parent, id, title, pos, size, style )
{
    ...
    wxMenu* fileMenu;
    fileMenu = new wxMenu();
    wxMenuItem* menuFileOpenFilm = new wxMenuItem(fileMenu, idMenuOpenFilm, wxString( wxT("&Open Film")), wxT("Open a video film"), wxITEM_NORMAL);
    fileMenu->Append( menuFileOpenFilm);
